...
function diplomaFields(d={}){
  const options=diplomaState.catalog.olympiads.map(o=>`<option value="${o.olympiad_id}" ${o.olympiad_id===d.olympiad_id?'selected':''}>${esc(o.name)} · ${esc(o.profile)} · ${esc(o.category==='rsosh'?'Перечневая':o.category==='vsosh'?'ВсОШ':o.category)}</option>`).join('');
