...
const quantity = value => value == null ? 'Нет данных' : num(value);
const tuition = p => p.cost == null ? (p.paid_places === 0 ? 'Нет платного набора' : 'Нет данных') : `${num(p.cost)} ₽/${p.cost_period === 'semester' ? 'семестр' : 'год'}`;
function quantitySummary(p) {
  if (!p.quantity_release) return '';
  return `<div class="program-quantities"><span>Бюджет: <strong>${quantity(p.budget_places)}</strong></span><span>Платно: <strong>${quantity(p.paid_places)}</strong></span><span>${tuition(p)}</span></div>`;
}
function quantitySection(p) {
  if (!p.quantity_release) return '';
  const titles = {budget_places:'Бюджетные места', paid_places:'Платные места', cost:'Стоимость обучения'};
  return `<section class="rule"><h2>Места и стоимость</h2><dl class="rule-facts">${Object.keys(titles).map(k=>`<div><dt>${titles[k]}</dt><dd>${k==='cost'?tuition(p):quantity(p[k])}</dd></div>`).join('')}</dl></section>`;
}
...
const route = () => location.pathname.split('/').filter(Boolean).map(part => { try { return decodeURIComponent(part); } catch { return part; } });
...
function olympiadCard(o) {
  if(o.academic_year)return `<article class="card"><span class="code">Уровень ${'I'.repeat(o.level)}</span><h3><a href="${href('olympiads',o.id)}">${esc(o.name)}</a></h3><p><strong>Профиль:</strong> ${esc(o.profile)}</p><p class="hint">${esc(o.subjects)}</p><div class="card-footer">${bookmark('olympiads',o.id,o.name)}<a class="arrow-link" href="${href('olympiads',o.id)}">${icon('arrow')}</a></div></article>`;
 return `<article class="card"><div class="card-top"><div class="olymp-mark">${icon('trophy')}</div>${bookmark('olympiads',o.id,o.name)}</div><h3><a href="${href('olympiads',o.id)}">${esc(o.name)}</a></h3><p class="card-description">${esc(o.profiles.slice(0,3).join(' · '))}${o.profiles.length>3 ? ` · ещё ${o.profiles.length-3}` : ''}</p><div class="tags"><span class="tag">${category[o.category]}</span>${benefits(o.benefit_types)}</div><div class="card-footer"><span>Вузов с условиями: <strong>${o.university_ids.length}</strong></span><a class="arrow-link" href="${href('olympiads',o.id)}">Льготы ${icon('arrow')}</a></div></article>`; }
...
function directory(type) {
  ...
  if (type==='olympiads') {
    const list=catalog.olympiads.filter(o=>matches([o.name,o.profile,o.subjects].join(' '))&&(!val('level')||String(o.level)===val('level'))&&(!val('profile')||o.profile===val('profile'))&&(!val('saved')||saved.has('olympiads:'+o.id)));
    const status=catalog.olympiad_registry?note(`РСОШ 2026/2027 · Проект перечня. ${external(catalog.olympiad_registry.source_url,'Официальный проект')}`):note('Перечень РСОШ 2026/2027 ещё не загружен.');
    return intro('Олимпиады РСОШ','Выбери профиль олимпиады и посмотри её уровень.','trophy')+status+`<div class="toolbar">${search('Олимпиада, профиль или предмет')}${select('level','Все уровни',[['1','I'],['2','II'],['3','III']])}${select('profile','Все профили',[...new Set(catalog.olympiads.map(o=>o.profile))].sort())}${favToggle()}</div>`+heading('Профили олимпиад',list.length)+paginate(list,olympiadCard,'grid directory-olymp');
  }
  const list=catalog.fields.filter(f=>f.university_ids.length>0&&matches(f.name+' '+f.code+' '+f.group)&&entityMatches(f,type)&&(!val('degree')||f.degree===val('degree')));
...
function universityPage(u) {
  ...
  let content=crumbs([['Вузы','/universities'],[u.short_name]])+hero({title:u.short_name,subtitle:u.name,uid:u.id,type:'universities',id:u.id,meta:`<span class="location">${icon('pin')}${esc(u.city)}</span>${external(u.site,'Сайт вуза')}${u.email?`<a href="mailto:${esc(u.email)}">${esc(u.email)}</a>`:''}<a class="button primary" href="${href('universities',u.id)}?tab=scholarships">Стипендии и поддержка</a><span>${num(u.rule_count)} условий льгот</span><span>${u.unit_count} подразделений · ${u.field_count} направлений</span><div class="tags">${benefits(u.benefit_types)}</div>`})+tabs([['units','Подразделения',u.unit_count],['programs','Все программы',u.program_count],['rules','Льготы',u.rule_count],['scholarships','Стипендии'],['about','О данных']],tab);
  if(tab==='rules')content+=ruleSection({university:u.id});
  else if(tab==='scholarships')content+=scholarshipSection(u.id);
  else if(tab==='about')content+=`<section class="rule source-summary"><h2>Что есть в каталоге</h2><p><strong>Охват:</strong> ${esc(u.scope)}</p><p><strong>Год приёма:</strong> ${catalog.admission_year}<br><strong>Дата сбора:</strong> ${esc(catalog.collected_on)}<br><strong>Условий льгот:</strong> ${num(u.rule_count)}<br><strong>Названий олимпиад:</strong> ${u.olympiad_ids.length}</p><p>У каждой записи есть ссылка на документ вуза и сохранённую копию с номером страницы. Стоимость и места приёма 2026 года приведены в карточках программ со ссылками на официальные источники. Общие конкурсные группы помечены отдельно.</p><a class="button primary" href="${href('universities',u.id)}?tab=rules">Смотреть условия ${icon('arrow')}</a></section>`;
...
function olympiadPage(o) {
  if(o.academic_year)return crumbs([['Олимпиады','/olympiads'],[o.name]])+hero({title:o.name,subtitle:o.profile,type:'olympiads',id:o.id,graphic:'trophy',meta:`<span>Уровень ${'I'.repeat(o.level)}</span><span>РСОШ ${esc(o.academic_year)} · Проект перечня</span>${external(o.source_url,'Официальный проект')}`})+`<section class="rule"><h2>Предметы и направления</h2><p>${esc(o.subjects)}</p></section>`+note('Условия льгот для дипломов этого сезона пока не подтверждены. Здесь появятся программы после проверки правил вузов.');
...
function favoritesPage() {
  const us=catalog.universities.filter(u=>saved.has('universities:'+u.id)),os=Object.values(olymp).filter(o=>saved.has('olympiads:'+o.id)),ps=catalog.programs.filter(p=>saved.has('programs:'+p.id));
...
function render() {
  ...
  document.querySelector('.year-label').textContent=type==='olympiads'&&!id||olymp[id]?.academic_year?'РСОШ 2026/2027':'Приём '+catalog.admission_year;
  ...
  else if(type==='olympiads'&&olymp[id]){content=olympiadPage(olymp[id])+`<a class="button" href="/calendar?scope=catalog&olympiad=${encodeURIComponent(id)}">События олимпиады →</a>`;title=olymp[id].name;}
  else if(type==='fields'&&fields[id]){content=fieldPage(fields[id]);title=fields[id].name;}
  else if(type==='programs'&&programs[id]){content=programPage(programs[id])+`<a class="button" href="/calendar?scope=catalog&program=${encodeURIComponent(id)}">Календарь поступления →</a>`+scholarshipSection(programs[id].university_id,id);title=programs[id].name;}
  else if(['calendar','diplomas'].includes(type)&&!id){content=personalPage(type);title=type==='calendar'?'Мой календарь':'Мои дипломы';}
  ...
  if(["calendar","diplomas"].includes(type)&&!id)renderPersonal(type,version);
}
...
function setFilter(key,value,replace=false) {const p=params();value?p.set(key,value):p.delete(key);if(key!=='page')p.delete('page');if(key!=='offset')p.delete('offset');navigate(location.pathname+(p.size?'?'+p:''),replace,key==='page');}
...
(async()=>{try{const response=await fetch('/api/catalog');if(!response.ok)throw new Error('Catalog unavailable');catalog=await response.json();uni=Object.fromEntries(catalog.universities.map(u=>[u.id,u]));olymp=Object.fromEntries([...(catalog.historical_olympiads||[]),...catalog.olympiads].map(o=>[o.id,o]));fields=Object.fromEntries(catalog.fields.map(f=>[f.id,f]));programs=Object.fromEntries(catalog.programs.map(p=>[p.id,p]));units=Object.fromEntries((catalog.units||[]).map(t=>[t.id,t]));shell();render();}catch(error){document.querySelector('#app').innerHTML='<div class="initial error"><h1>Не удалось открыть каталог</h1><p>Убедитесь, что сервер запущен командой python web/server.py, и обновите страницу.</p></div>';}})();
