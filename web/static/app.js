...
function quantitySummary(p) {
  ...
  return `<div class="program-quantities"><span>Бюджет: <strong>${quantity(p.budget_places)}</strong></span><span>Платно: <strong>${quantity(p.paid_places)}</strong></span><span>${tuition(p)}</span></div><div class="hint">Приём ${p.admission_year} · ${esc(p.study_form)}${p.places_note ? ' · Места на общий набор' : ''}</div>`;
}
function quantitySection(p) {
  if (!p.quantity_release) return '';
  const titles = {budget_places:'Бюджетные места', paid_places:'Платные места', cost:'Стоимость обучения'};
  return `<section class="rule"><h2>Места и стоимость · ${p.admission_year}</h2><p>${esc(p.study_form)} · Первый курс · Бюджетные места включают квоты</p><dl class="rule-facts">${Object.keys(titles).map(k=>`<div><dt>${titles[k]}</dt><dd>${k==='cost'?tuition(p):quantity(p[k])}</dd></div>`).join('')}</dl>${p.places_note?note(esc(p.places_note)):''}${(p.quantity_notes||[]).map(t=>note(esc(t))).join('')}<div class="rule-source">${Object.entries(p.quantity_evidence||{}).map(([key,e])=>external(e.source_url,`Источник: ${titles[key].toLowerCase()}`)).join('')}</div></section>`;
}
...
function universityPage(u) {
  ...
  let content=crumbs([['Вузы','/universities'],[u.short_name]])+hero({title:u.short_name,subtitle:u.name,uid:u.id,type:'universities',id:u.id,meta:`<span class="location">${icon('pin')}${esc(u.city)}</span>${external(u.site,'Сайт вуза')}<span>${num(u.rule_count)} условий льгот</span><span>${u.unit_count} подразделений · ${u.field_count} направлений</span><div class="tags">${benefits(u.benefit_types)}</div>`})+tabs([['units','Подразделения',u.unit_count],['programs','Все программы',u.program_count],['rules','Льготы',u.rule_count],['scholarships','Стипендии'],['about','О данных']],tab);
